...
async function submitFirebase(name){
  const {dbMod,userRef}=firebaseApi;

  const slot = String(count + 1);

  if (Number(slot) > cfg.MAX_SUBMISSIONS) {
    throw new Error('LIMIT');
  }

  const itemRef = dbMod.child(userRef, slot);

  await dbMod.set(itemRef, {
    name,
